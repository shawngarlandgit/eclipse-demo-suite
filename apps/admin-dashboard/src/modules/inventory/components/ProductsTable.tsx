      ...
      <Table variant="simple" size="sm">
        ...
        <Tbody>
          {products.map((product) => {
            // Handle both Convex (camelCase) and Supabase (snake_case) field names
            const quantityOnHand = product.quantityOnHand ?? product.quantity_on_hand ?? 0;
            ...
            return (
              <Tr
                ...
                onClick={() => openDetailModal(productId)}
              >
                {/* Product Info */}
                <Td>
                  <HStack spacing={3}>
                ...
                {/* Strain Type */}
                <Td>
                  {'strain_type' in product && product.strain_type ? (
                    <Badge
                      bg={
                        product.strain_type === 'indica' ? '#9333EA' :
                        product.strain_type === 'sativa' ? '#F97316' :
                        product.strain_type === 'hybrid' ? '#06B6D4' :
                        (product.strain_type as string)?.toLowerCase().includes('indica') ? '#EC4899' :
                        (product.strain_type as string)?.toLowerCase().includes('sativa') ? '#EF4444' :
                        '#22D3EE'
                      }
                      color="white"
                      px={2}
                      ...
                      fontWeight="bold"
                    >
                      {product.strain_type as string}
                    </Badge>
                  ) : (
                    <Text fontSize="xs" color="slate.500">-</Text>
                  )}
                </Td>
                ...
                <Td>
                  <Text fontSize="sm" color="slate.300" textTransform="capitalize">
                    {(product.category || product.product_type || 'other').replace('_', ' ')}
                  </Text>
                </Td>
                ...
                <Td>
                  ...
                  {(product.costPrice || product.cost) && (
                    <Text fontSize="xs" color="slate.400">
                      Cost: {formatCurrency(product.costPrice ?? product.cost)}
                    </Text>
                  )}
                </Td>
                  ...
                  <Menu>
                    ...
                    <MenuList>
                      ...
                      <MenuItem
                        ...
                        onClick={() => openAdjustmentDrawer(productId)}
                      >
                        Adjust Stock
                      </MenuItem>
