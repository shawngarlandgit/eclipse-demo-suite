...
function ProductDetailModal() {
  ...
  const stockInfo = product
    ? inventoryService.getStockLevelInfo(
        ...
        product.lowStockThreshold ?? product.low_stock_threshold ?? product.reorder_level ?? 10
      )
    : null;
        ...
        <ModalHeader color="white" borderBottom="1px" borderColor="slate.700">
          {isLoading ? (
            ...
            <HStack spacing={3}>
              ...
              <Badge
                bg={(product?.isActive ?? product?.is_active ?? true) ? '#10B981' : '#EF4444'}
                color="white"
                px={3}
                py={1}
                borderRadius="md"
                ...
                fontWeight="bold"
              >
                {(product?.isActive ?? product?.is_active ?? true) ? 'Active' : 'Inactive'}
              </Badge>
            </HStack>
          )}
        </ModalHeader>
            ...
            <VStack spacing={6} align="stretch">
                  ...
                  <VStack spacing={3} align="stretch">
                    <Box>
                      ...
                      <Badge
                        ...
                        fontWeight="bold"
                      >
                        {(product.category || product.product_type || 'other').replace('_', ' ').toUpperCase()}
                      </Badge>
                    </Box>
              ...
              <Box>
                ...
                <Box bg="slate.750" p={4} borderRadius="md">
                  <CannabinoidProfile
                    thc_pct={product.thcPercentage ?? product.thc_pct}
                    cbd_pct={product.cbdPercentage ?? product.cbd_pct}
                    cbg_pct={product.cbgPercentage ?? product.cbg_pct}
                    thca_pct={product.thcaPercentage ?? product.thca_pct}
                    layout="vertical"
                    size="md"
                  />
                </Box>
                ...
                <Grid templateColumns={{ base: '1fr', md: 'repeat(2, 1fr)' }} gap={4}>
                  <Box bg="slate.750" p={4} borderRadius="md">
                    ...
                    {stockInfo && (
                      <StockLevelIndicator
                        stockInfo={stockInfo}
                        quantity={product.quantityOnHand ?? product.quantity_on_hand ?? 0}
                        showLabel={true}
                        size="md"
                      />
                    )}
                    ...
                    <VStack spacing={2} align="stretch">
                      <Flex justify="space-between">
                        ...
                        <Text fontSize="md" fontWeight="bold" color="white">
                          {product.lowStockThreshold ?? product.reorder_level ?? 10} units
                        </Text>
                      </Flex>
                      {(product.lowStockThreshold || product.low_stock_threshold) && (
                        <Flex justify="space-between">
                          ...
                          <Text fontSize="md" fontWeight="bold" color="white">
                            {product.lowStockThreshold ?? product.low_stock_threshold} units
                          </Text>
                        </Flex>
                      )}
                      <Flex justify="space-between">
                        ...
                        <Badge
                          ...
                          fontWeight="bold"
                        >
                          {product.batch_count ?? product.batchCount ?? 1} {(product.batch_count ?? product.batchCount ?? 1) === 1 ? 'batch' : 'batches'}
                        </Badge>
                      </Flex>
                    </VStack>
                  </Box>
                  ...
                  <Tbody>
                    <Tr>
                      ...
                      <Td color="white" isNumeric>
                        {formatCurrency((product.retailPrice ?? product.price ?? 0) * (product.quantityOnHand ?? product.quantity_on_hand ?? 0))}
                      </Td>
                    </Tr>
                    {(product.costPrice || product.cost) && (
                      <>
                        <Tr>
                          ...
                          <Td color="white" isNumeric>
                            {formatCurrency((product.costPrice ?? product.cost ?? 0) * (product.quantityOnHand ?? product.quantity_on_hand ?? 0))}
                          </Td>
                        </Tr>
                        <Tr bg="slate.750">
                          <Td color="slate.300" fontWeight="bold">Margin</Td>
                          <Td color="cannabis.400" fontWeight="bold" isNumeric>
                            {formatCurrency((product.retailPrice ?? product.price ?? 0) - (product.costPrice ?? product.cost ?? 0))}
                          </Td>
                          <Td color="cannabis.400" fontWeight="bold" isNumeric>
                            {((((product.retailPrice ?? product.price ?? 0) - (product.costPrice ?? product.cost ?? 0)) / (product.retailPrice ?? product.price ?? 1)) * 100).toFixed(1)}%
                          </Td>
                        </Tr>
                      </>
                    )}
              ...
              <Box>
                ...
                <Grid templateColumns={{ base: '1fr', md: 'repeat(2, 1fr)' }} gap={4}>
                  ...
                  {(product.updatedAt || product.last_updated) && (
                    <Box bg="slate.750" p={4} borderRadius="md">
                      <Text fontSize="sm" color="slate.400" mb={2}>
                        Last Updated
                      </Text>
                      <Text fontSize="sm" color="white">
                        {formatDate(product.updatedAt ?? product.last_updated)}
                      </Text>
                    </Box>
                  )}

                  {(product.lastRestockedAt || product.last_restocked) && (
                    <Box bg="slate.750" p={4} borderRadius="md">
                      <Text fontSize="sm" color="slate.400" mb={2}>
                        Last Restocked
                      </Text>
                      <Text fontSize="sm" color="white">
                        {formatDate(product.lastRestockedAt ?? product.last_restocked)}
                      </Text>
                    </Box>
                  )}
                </Grid>
