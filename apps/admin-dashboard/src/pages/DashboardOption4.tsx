import { Box, Heading, Text, Flex, VStack, HStack, Badge, Grid } from '@chakra-ui/react';
    ...
    <Box>
      <HStack justify="space-between" mb={4}>
        ...
        <HStack spacing={3}>
          ...
          <EditModeToolbar dashboardStyle="option-4" />
        </HStack>
      </HStack>

      {/* Draggable Grid Layout */}
      <DraggableGrid dashboardStyle="option-4">
        {/* Left Sidebar - Metrics */}
        <DraggableWidget key="sidebar-metrics" id="sidebar-metrics" noPadding>
          <Box bg="slate.800" borderRadius="md" p={4} h="100%" overflow="auto">
        ...
        {/* Main Content Area - Revenue Chart */}
        <DraggableWidget key="main-chart" id="main-chart" noPadding>
          <Box h="100%">
            <RevenueChart data={salesTrend || []} isLoading={trendLoading} />
          </Box>
        </DraggableWidget>

        {/* Lists Section - Products and Staff */}
        <DraggableWidget key="lists-section" id="lists-section" noPadding>
          <Grid templateColumns={{ base: '1fr', lg: '1fr 1fr' }} gap={4} h="100%">
            {/* Top Products */}
            <Box bg="slate.800" borderRadius="md" overflow="hidden" h="100%">
                ...
                {topProducts?.map((product, idx) => (
                  ...
                  >
                    ...
                    <Box textAlign="right">
                      ...
                      <Text fontSize="xs" color="slate.500">{(product.margin || 0).toFixed(0)}% margin</Text>
                    </Box>
                  </Flex>
                ))}
              </VStack>
              ...
              <VStack spacing={0} align="stretch">
                {staffPerformance?.slice(0, 5).map((staff, idx) => (
                  ...
                  >
                    ...
                    <Box textAlign="right">
                      <Text fontSize="sm" fontWeight="bold" color="green.400">{formatCurrency(staff.sales)}</Text>
                      <Text fontSize="xs" color="slate.500">{formatCurrency(staff.avg_transaction_value)} avg</Text>
                    </Box>
                  </Flex>
                ))}
              </VStack>
