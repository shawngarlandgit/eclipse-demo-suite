...
class MockInventoryService {
  ...
  private generateMockData() {
    ...
    this.strainsFromDB.forEach((strain, index) => {
      ...
      const product: ProductWithInventory = {
        ...
        reorder_level: Math.floor(threshold * 1.5),
        price: parseFloat(basePrice.toFixed(2)),
        cost: parseFloat((basePrice * 0.6).toFixed(2)),
        is_active: Math.random() > 0.1, // 90% active
        thc_pct: thcPercentage,
      ...
      for (let b = 0; b < batchCount; b++) {
        ...
        const batchQuantity = Math.floor(quantity / batchCount);
        const batch: ProductBatch = {
          ...
          quantity_initial: batchQuantity,
  ...
  async getProducts(filters?: any, dateRange?: { startDate: string; endDate: string }): Promise<ProductWithInventory[]> {
    ...
    if (filters?.stockLevel && filters.stockLevel !== 'all') {
      filtered = filtered.filter(p => {
        const stockInfo = this.getStockLevelInfo(p.quantity_on_hand, p.low_stock_threshold || 0);
  ...
  async getInventorySummary(dateRange?: { startDate: string; endDate: string }): Promise<InventorySummary> {
    ...
    const totalCostValue = this.products.reduce((sum, p) => sum + (p.quantity_on_hand * (p.cost ?? 0)), 0) * scaleFactor;

    const lowStockCount = this.products.filter(p =>
      p.is_active &&
      p.quantity_on_hand > 0 &&
      p.quantity_on_hand <= (p.low_stock_threshold || 0)
    ).length;
    ...
    const expiringSoonCount = this.batches.filter(b => {
      const expDate = new Date(b.expiration_date || new Date().toISOString());
  ...
  async getComplianceAlerts(): Promise<ComplianceAlert[]> {
    ...
    this.batches.forEach(batch => {
      const expDate = new Date(batch.expiration_date || new Date().toISOString());
    ...
    // Low stock alerts
    this.products.filter(p => p.is_active && p.quantity_on_hand <= (p.low_stock_threshold || 0)).forEach(product => {
      alerts.push({
